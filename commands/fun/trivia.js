module.exports = {
    ...
    run: async (bot, message, args) => {
        ...
        class Game {
            ...
            let levels = ["any", "easy", "medium", " hard" ]  
                    let question
                    if (dif.toLowerCase() == 'any') return this.get_data()
                    if (dif.toLowerCase() != 'easy' && dif.toLowerCase() != 'medium' && dif.toLowerCase() != 'hard') return this.message.channel.send('Please enter a valid difficulty level.\n'+`**\`${levels.join("\`** / **\`")}\`**`);
                ...
                this.question_message.awaitReactions((reaction, user) => user.id == message.author.id && (reaction.emoji.name == '🇦' || reaction.emoji.name == '🇧' || reaction.emoji.name == '🇨' || reaction.emoji.name == '🇩'),
                    { max: 1, time: 30000 }).then(collected => {
                        ...
                        if (this.input_answer == this.correct_answer) {
                            ...
                            this.answer_array[this.input_answer - 1] = this.answer_array[this.input_answer - 1] + ':x:'
                            this.question_embed = new discord.MessageEmbed()
                                .setColor('RANDOM')
                                .setTitle(atob(this.question.results[0].question))
                                .setDescription(this.answer_array)
                                .setFooter('Category - ' + atob(this.question.results[0].category) + ', Difficulty - ' + atob(this.question.results[0].difficulty))
                            this.question_message.edit(this.question_embed)
                            this.question_message.edit(':x: You got it wrong. The correct answer was ' + this.reactions[this.correct_answer - 1])
                            this.end_game()
                        }
                    }).catch(() => {
                        this.question_message.edit(':x: You took to long to answer. Game has timed out. The answer was ' + this.reactions[this.correct_answer - 1])
                        this.end_game()
                    })
            }
            async end_game() {
